...
def gramm_Hulk_LR1():
    ...
    (
        ...
        "print_statement assignment function_definition control_structure contElif contElse exp_or_cond"
    )
    (
        if_structure,
        while_structure,
        ...
        "if_structure while_structure for_structure member method_override create_statement non_create_statement"
    )
    (
        compAritCond,
        compBoolCond,
        ...
        "compAritCond compBoolCond base_args let_in multi_assignment kern_assignment op_factor op_term"
    )
    ...
    identifier, number, string, Elif, Type, Inherits, New, In, def_Type = G.Terminals(
        "identifier number string elif type inherits new in def_Type"
    )
    ...
    multi_assignment %= (
        kern_assignment + Colon + multi_assignment | kern_assignment + Semi
    )
    ...
    condition %= (
        expression + compAritCond + expression
        | Not + condition
        | condition + compBoolCond + condition
        | _True
        | _False
        | identifier + Is + def_Type
        | oPar + condition + cPar
        | function_call
        | identifier
    )
    ...
    attribute_definition %= (
        identifier + type_annotation + Equal + exp_or_cond + Semi + attribute_definition
        | G.Epsilon
    )
    method_definition %= (
        identifier
    ...
    #
    method_override %= (
        identifier + oPar + parameters + cPar + oBrace + statement_list + Semi + cBrace
        | G.Epsilon
    )
